...
class CI_DB_sqlsrv_driver extends CI_DB {
	...
	function _delete($table, $where = NULL)
	{
		...
		return "DELETE FROM ".$this->_escape_table($table). $where_str;
	}
	...
	function _limit($sql, $limit, $offset)
	{
        ...
        if (count($this->ar_orderby) > 0) // If called as $this->db->limit(100,200);
        {
            ...
            $sql = preg_replace('/(^\SELECT (DISTINCT)?)/i','\\1', $sql);
            
            $sql = "SELECT *, row_number() OVER (".$OrderBy.") AS rownum FROM (" . $sql . ") AS B1";
		...
		$rs = preg_replace('/(^\SELECT (DISTINCT)?)/i','\\1 TOP '.$i.' ', $sql);		
		return $rs;*/
	}
