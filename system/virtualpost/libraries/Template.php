...
class Template
{
	...
	private $_header_title_separator = ' &raquo; ';
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if (empty($this->_header_title))
		{
		    $this->_header_title = $this->_guess_header_title();
		}

		// Output template variables to the template
		$template['title']	= $this->_title;
		if (empty($this->_data['header_title'] )) {
		    $template['header_title']	= $this->_header_title;
		} else {
		    $template['header_title']	= $this->_data['header_title'];
		}
	...
	private function _guess_header_title()
	{
	    $this->_ci->load->helper('inflector');
	    ...
	    // Make sure controller name is not the same as the method name
	    if ( ! in_array($this->_controller, $title_parts) && $this->_controller != 'admin')
	    {
	        $title_parts[] = $this->_controller;
	    }
	    ...
	    // Glue the title pieces together using the title separator setting
	    $title = humanize(implode($this->_header_title_separator, $title_parts));
