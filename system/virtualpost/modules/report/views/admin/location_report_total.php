<div class="header">
    <h2 style="font-size: 20px; margin-bottom: 10px"><?php admin_language_e('report_views_admin_location_report_total_Header'); ?></h2>
</div>
        ...
        <div class="ym-gl">
            <div class="ym-grid input-item">
                <div class="ym-g30 ym-gl" style="width: 100px">
                    <label style="text-align: left;"><?php admin_language_e('report_views_admin_location_report_total_LblLocation'); ?></label>
                </div>
                <div class="ym-g70 ym-gl">
                    ...
                    <button style="margin-left: 30px"
                            id="locationReportingButton" class="admin-button"><?php admin_language_e('report_views_admin_location_report_total_LblSearch'); ?></button>
                    <?php if(APContext::isSupperAdminUser()): ?>
                         <button id="generateReport" type="button"  class="admin-button"><?php admin_language_e('report_views_admin_location_report_total_LblGenReport'); ?></button>
                         <button id="generateInvoiceTotalButton" type="button" class="admin-button"><?php admin_language_e('report_views_admin_location_report_total_LblUpdateReport'); ?></button>
<!--                    
    ...
    /** START SOURCE TO VIEW LOCATION REPORT DETAIL AND DIRECT CHARGE */
        include 'system/virtualpost/modules/report/views/admin/partial/location_report_total_detail.php';
    /** END SOURCE TO VIEW LOCATION REPORT DETAIL AND DIRECT CHARGE */
     ?>
<div class="clear-height"></div>
<!-- Content for dialog -->
<div class="hide"  style="display: none;">
	<div id="createReport" title="<?php admin_language_e('report_views_admin_location_report_total_TitGenReport'); ?>" class="input-form dialog-form">
	</div>
    <div id="divCreditNoteContainer" title="<?php admin_language_e('report_views_admin_location_report_total_TitCreditNot'); ?>" class="input-form dialog-form"></div>
</div>
<div class="hide" style="display: none;">
    <a id="view_report_file" class="iframe"><?php admin_language_e('report_views_admin_location_report_total_PreviewFile'); ?></a>
    <a id="display_pdf_invoice" class="iframe" href="#"><?php admin_language_e('report_views_admin_location_report_total_DisPdfInv'); ?></a>
</div>
    ...
    $(document).ready(function () {
        ...
        $('#generateReport').click(function() {
            ...
            $('#createReport').openDialog({
                ...
                buttons: {
                    '<?php admin_language_e('report_views_admin_location_report_total_BtnGenReportDlg'); ?>': function() {
                        createReport();
                    }
        ...
        */
        ...
        $("#generateInvoiceTotalButton").click(function(){
            location.href = "<?php echo base_url() ?>admin/report/update_location_report?ym=<?php echo date("Ym", now()); ?>";
        });
        ...
        $("#location_available_id").live('change', function(){
            hideUpdateButton();
        });
        ...
        function hideUpdateButton(){
            if($.trim($("#location_available_id").val()) != '' || $("#year").val() != '<?php echo date("Y") ?>'
                    || $("#month").val() != '<?php echo date("m") ?>'){
                $("#generateInvoiceTotalButton").hide();
            }else{
                $("#generateInvoiceTotalButton").show();
            }
        }
        ...
        $("#creditNoteListButton").click(function(e){
            ...
            $('#divCreditNoteContainer').openDialog({
                ...
                buttons: {
                    '<?php admin_language_e('report_views_admin_location_report_total_BtnCloseDlg'); ?>': function () {
                        $(this).dialog('close');
                    }
                }
            });
