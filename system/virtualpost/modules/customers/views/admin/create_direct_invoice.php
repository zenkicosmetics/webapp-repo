<div class="button_container mailbox">
    <table style="border: 0px solid #dadada;margin: 5px 10px;width: 750px">
        <tr>
            ...
            <td><input type="text" value="<?php echo date($date_format); ?>" id="createDirectChargeDetailResult_InvoiceDate" class="input-width input_date" style="width: 100px" /></td>
    ...
    $list_direct_desc = Settings::get_list(APConstants::LIST_DESC_MANUAL_INVOICES);
    ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var list_desc = [];
        <?php foreach ($list_direct_desc as $desc) { ?>
                    list_desc.push('<?php echo $desc->ActualValue; ?>');
        <?php } ?>
        ...
        function createDirectCharge() {
            for (var i = 0; i < 1; i++) {
                data.push({id: i + 1, description: '', quantity: '', net_price: ''});
            }
            ...
            $("#dataGridDirectChargeDetailResult").jqGrid({
                ...
                colModel: [
                    ...
                    {name: 'description', editable: true, index: 'description', sortable: false, width: 370, edittype: 'custom', editoptions: {custom_element: autocomplete_element, custom_value: autocomplete_value}},
            ...
            function autocomplete_element(value, options) {
                ...
                var $ac = $('<input type="text"/ style="width:98%">');
                // setting value to the one passed from jqGrid
                $ac.val(value);
                // creating autocomplete
                $ac.autocomplete({source: list_desc, minLength: 0});
