...
<form id="creditCardPaymentForm" method="post" class="dialog-form" action="<?php echo base_url() ?>customers/save_paypal_payment">
    <div style="margin: auto 0;">
        <table style=" width: <?php echo $width ?>; margin:10px;">
            <!-- only show for thank page--->
            <?php if($thank_page == 1){ ?>
            <tr><td colspan="3" style="text-align: left">How much do you want to deposit into your account?</td></tr>
            <tr style="height: 25px;">
                <td style="vertical-align: middle;text-align: left" colspan="2" >
                    <input type="text" id="creditCardPaymentForm_paypal_amount" name="paypal_amount" value="<?php echo floatval($pre_total_amount) != 0 ? APUtils::number_format($pre_total_amount) : "";?>" class="input-txt-none" style="width: 90px;" />&nbsp;&nbsp;EUR
                </td>
                <td style="vertical-align: middle;text-align: right">
                    <a href="#" id="paypalPaymentForm_paynowWithPaypal01"   >
                        <img src="<?php echo APContext::getImagePath()?>/paypal.gif" alt="Check out with PayPal" style="width: 120px" />
                    </a>
                    <a href="#" id="paypalPaymentForm_paynowWithPaypal01"  <?php if ($open_balance <= 0 && $action_type !== 'delete_postbox'): ?>style="display:none;"<?php endif; ?>>
                        <img src="<?php echo APContext::getImagePath()?>/paypal.gif" alt="Check out with PayPal" style="width: 120px" />
                    </a>
                </td>
            </tr>
            ...
            <tr style="height: 25px;">
                <td  colspan="2" class="input-width" style="vertical-align: middle;">
                    <span style="float: left; margin-top: 5px;">Show in other currency:&nbsp;&nbsp;</span>
                </td>
                <td style="text-align: right">
                    <span id="converted_amount_02" style="margin-top: 5px; margin-right: 8px;">0,00</span> 
                    <select name="currency_id_02" id="currency_id_02" style="width: 100px; margin-right: 10px; margin-top:1px;" class="input-txt-none">
            ...
            <!-- Show for normal payment with paypal--->
            <?php }else{?>
            <tr><td colspan="3">Do you want to make a PayPal payment for your open balance:</td></tr>
            <tr style="height: 25px;">
                <td style="vertical-align: middle;">
                    <?php if ($action_type !== 'delete_postbox') { ?>
                    Open balance due:&nbsp;&nbsp;&nbsp;
                    <?php } else {?>
                    Open balance:&nbsp;&nbsp;&nbsp;
                    <?php }?>
                    <br />
                    <span id="base_amount_01" style="font-weight: bold;">
                        <?php 
                        if ($action_type !== 'delete_postbox') { 
                            echo APUtils::convert_currency($open_balance, 1, 2, $decimal_separator);
                        }else {
                            echo APUtils::convert_currency($pre_total_amount, 1, 2, $decimal_separator); 
                        }
                        ?>
                    </span>
                ...
                <td class="input-width" style="vertical-align: middle; min-width: 365px; padding-left: 12px; display: block; height: 30px; margin-top:10px;">
                    <span style="float: left; margin-top: 5px;">Show in other currency:&nbsp;&nbsp;</span>
                    <select name="currency_id_01" id="currency_id_01" style="float: right;width: 100px; margin-right: 10px; margin-top:1px;" class="input-width">
                        <?php foreach($currencies as $currency): ?>
                            <?php if ($currency->currency_id == $selected_currency->currency_id): ?>
                                <option value="<?php echo $currency->currency_id; ?>" selected="selected"><?php echo $currency->currency_short; ?></option>
                            <?php else: ?>
                    ...
                    <?php if ($action_type !== 'delete_postbox') { ?>
                    <span id="converted_amount_01" style="float: right;margin-top: 5px; margin-right: 8px;"><?php echo APUtils::convert_currency($open_balance, $selected_currency->currency_rate, 2, $decimal_separator); ?></span>
                     <?php } else {?>
                    <span id="converted_amount_01" style="float: right;margin-top: 5px; margin-right: 8px;"><?php echo APUtils::convert_currency($pre_total_amount, $selected_currency->currency_rate, 2, $decimal_separator); ?></span>
                     <?php }?>
                </td>
                
            </tr>
            <tr <?php if ($action_type === 'delete_postbox' && $total_postbox == 1) {?>  style="display: none"  <?php } ?> ><td colspan="3">Or do you want to make a deposit payment into your account:</td></tr>
            <tr style="height: 25px; <?php if ($action_type === 'delete_postbox' && $total_postbox == 1) {?>display: none<?php } ?>">
                <td style="vertical-align: middle;">
                    <input type="text" id="creditCardPaymentForm_paypal_amount" name="paypal_amount" value="<?php echo floatval($pre_total_amount) != 0 ? APUtils::number_format($pre_total_amount) : "";?>" class="input-txt-none" style="width: 100px;" />&nbsp;&nbsp;EUR
                </td>
                <td style="vertical-align: middle;">
                ...
                <td id="currency-conversion-box" class="input-width" style="vertical-align: middle; min-width: 365px; padding-left: 12px; display: block; height: 30px; margin-top:10px;">
                    <span style="float:left; margin-top:5px;">Show in other currency:&nbsp;&nbsp;</span>
        ...
        <input type="hidden" id="creditCardPaymentForm_pre_total_amount" name="creditCardPaymentForm_pre_total_amount" value="<?php echo $pre_total_amount?>">
        <input type="hidden" id="creditCardPaymentForm_total_postbox" name="creditCardPaymentForm_total_postbox" value="<?php echo $total_postbox?>">
</form>
...
<script>
    ...
    ?>
    var total_postbox = '<?php echo $total_postbox;?>';
    var paypalButtons = ['paypalPaymentForm_paynowWithPaypal02'];
    <?php if ($open_balance > 0 || $action_type === 'delete_postbox'): ?>
    paypalButtons.push('paypalPaymentForm_paynowWithPaypal01');
    <?php endif; ?>

    $('#paypalPaymentForm_paynowWithPaypal01, #paypalPaymentForm_paynowWithPaypal02').live('click', function(){
        return false;
    });
    window.paypalCheckoutReady = function() {
        ...
        paypal.checkout.setup(merchantId, {
            ...
            buttons: [{
            	button: 'paypalPaymentForm_paynowWithPaypal01',
		...
		// Pay with default amount
        function payDefaultAmount() {
            ...
            if (action_type === 'delete_postbox') {
                paypal_amount = parseFloat($('#creditCardPaymentForm_pre_total_amount').val().replace(',', '.'));
            }
            var submit_url = "<?php echo base_url() ?>customers/save_paypal_payment?paypal_amount=" + paypal_amount;
        ...
        function payCustomAmount() {
            ...
            var paypal_amount = parseFloat($('#creditCardPaymentForm_paypal_amount').val().replace(',', '.'));
            var action_type = $('#creditCardPaymentForm_action_type').val();
            if (action_type === 'delete_postbox') {
                var min_paypal_amount = parseFloat($('#creditCardPaymentForm_pre_total_amount').val().replace(',', '.'));
                if (paypal_amount < min_paypal_amount) {
                    // Display error message
                    $.displayError('Please input valid amount. The amount should be numeric value and greater than or equal ' + min_paypal_amount + ' EUR');
                    return false;
                }
            }
            ...
            if (!$.isValidNumber(paypal_amount) || paypal_amount <= 0) {
                // Display error message
                $.displayError('Please input valid amount. The amount should be numeric value and greater than 0');
                return false;
            }
            var submit_url = "<?php echo base_url() ?>customers/save_paypal_payment?paypal_amount=" + paypal_amount;
			var prepayment = $('#creditCardPaymentForm_prepayment').val();
...
<script type="text/javascript">
    jQuery(document).ready(function($){
        ...
        var action_type = $('#creditCardPaymentForm_action_type').val();
        if (action_type === 'delete_postbox') {
            open_balance_due = parseFloat($('#creditCardPaymentForm_pre_total_amount').val().replace(',', '.'));
        }
        ...
        $("#creditCardPaymentForm_paypal_amount").bind('change', function(){
