 ...
 * @author DuNT
 */
class pricing_phones_number_customer_m extends MY_Model {
    function __construct() {
        parent::__construct();
        
        $this->_table = $this->profile_table = $this->db->dbprefix('pricing_phones_number_customer');
        $this->primary_key = 'id';
    }
    ...
    public function get_price_paging($array_where, $start = 0, $limit = 10, $sort_column, $sort_type = 'ASC')
    {
        ...
        $this->db->select('pricing_phones_number_customer.*');
        $this->db->select('country.country_name');

        $this->db->join('country', 'pricing_phones_number_customer.country_code_3 = country.country_code_3', "inner");
    ...
    {
        $this->db->select('COUNT(pricing_phones_number_customer.id) AS TotalRecord');
