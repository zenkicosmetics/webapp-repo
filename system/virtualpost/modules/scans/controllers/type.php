...
class Type extends Admin_Controller {
    ...
    private $validation_rules = array (
            array (
                    'field' => 'LabelValue',
                    'label' => 'lang:type.LabelValue',
                    'rules' => 'required|max_length[100]' 
            ),
            array (
                    'field' => 'Alias01',
                    'label' => 'lang:type.Customs',
                    'rules' => 'max_length[1]' 
            ),
            array (
                    'field' => 'Alias02',
    ...
    public function index() {
        ...
        if ($this->is_ajax_request()) {
            ...
            $this->template->set('header_title', lang('header:list_user_title'))->build('type/index');
        }
    }
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            $Alias01 = $this->input->post('Alias01');
            $Alias02 = $this->input->post('Alias02');
            $category_value = $this->setting_m->count_by_many(array (
                    'SettingCode' => APConstants::ENVELOPE_TYPE_CODE 
            )) + 1;
            
            if ($this->form_validation->run()) {
                $this->setting_m->insert(array (
                        'LabelValue' => $category_name,
                        'ActualValue' => $category_value,
                        'Alias01' => $Alias01,
                        'Alias02' => $Alias02,
                        'SettingCode' => APConstants::ENVELOPE_TYPE_CODE 
                ));
                
                $this->success_output(lang('type.add_successfull'));
                return;
            }
            else {
        ...
        $category->SettingKey = '';
        // Display the current page
        $this->template->set('category', $category)->set('action_type', 'add')->build('type/form');
    }
    ...
    public function edit() {
        ...
        if (! ($member = $this->setting_m->get_by("SettingKey", $SettingKey))) {
            $this->session->set_flashdata('error', lang('type.not_found'));
            echo lang('type.not_found');
            return;
        }
        if ($_POST) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                $update_data ['Alias01'] = $this->input->post('Alias01');
                $update_data ['Alias02'] = $this->input->post('Alias02');
                
                $result = $this->setting_m->update($SettingKey, $update_data);
                if ($result) {
                    $this->success_output(lang('type.edit_successfull'));
                    return;
                }
                else {
                    $this->error_output(lang('type.edit_not_successfull'));
                    return;
                }
            }
            else {
        ...
        // Display the current page
        $this->template->set('category', $member)->set('action_type', 'edit')->build('type/form');
    }
    ...
    public function delete() {
        ...
        if ($success) {
            $message = lang('type.delete_success');
            $this->success_output($message);
            return;
        }
        else {
            $message = lang('type.delete_error');
