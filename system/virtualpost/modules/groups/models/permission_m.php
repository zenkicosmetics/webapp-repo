...
class Permission_m extends MY_Model {
    ...
    public function get_group($group_id) {
        ...
        // Execute the query
        $result = $this->db->where('group_id', $group_id)->get('permissions')->result();
        ...
        foreach ( $result as $row ) {
            ...
            $rules [$row->module_name] = $row->roles ? json_decode($row->Roles, true) : TRUE;
        }
     ...
     * 
     * @param int $group_id
     *            The id for the group to get the rule for.
     * @param null|string $module
     *            The module to check access against
     * @return bool
     */
    public function check_access($group_id, $module = NULL) {
        ...
        return $this->db->where('group_id', $group_id)->count_all_results('permissions') > 0;
    }
    ...
    public function save($group_id, $modules, $module_roles) {
        ...
        if ($modules) {
            // For each module mentioned (with a value of 1 for most browser
            // compatibility).
