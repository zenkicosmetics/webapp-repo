...
class cases_taskname_m extends MY_Model
{
    public function __construct()
    {
        parent::__construct();

        $this->_table = $this->profile_table = $this->db->dbprefix('cases_taskname');
        $this->primary_key = 'id';
    }
     ...
     */
    public function get_task_paging($array_where, $start = 0, $limit = 10, $sort_column, $sort_type = 'ASC')
    {
        // Count all record with input condition
        $total_record = $this->count_by_task_paging($array_where);
        if ($total_record == 0) {
            return array(
                "total" => 0,
                "data" => array()
        ...
        $this->db->select('cases_taskname.*')->distinct();
        $this->db->select(
        ...
        $this->db->join('cases_milestone', 'cases_taskname.milestone_id = cases_milestone.id', 'inner');
        $this->db->join('cases_product', 'cases_product.id = cases_milestone.product_id', 'inner');
        $this->db->join('cases_service_partner', 'cases_service_partner.partner_id = cases_milestone.partner_id', 'inner');
     ...
     * @param unknown_type $array_where
     */
    public function count_by_task_paging($array_where)
    {
        $this->db->select('COUNT(DISTINCT(cases_taskname.id)) AS total_record');
