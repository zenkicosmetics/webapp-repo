 ...
 * customization. This is basically what Redux Auth 2 should be.
...
class Ion_auth_model extends My_Model
{
    ...
    public function hash_password_db($id, $password)
    {
        ...
        $query = $this->db->select('password')->select('salt')->where('id', $id)->where($this->ion_auth->_extra_where)->limit(1)->get($this->tables ['users']);
        ...
        if (!empty($hash_password_db->salt)) {
            if ($this->store_salt) {
    ...
    {
        $query = $this->db->select('password, salt')->where($this->identity_column, $identity)->where($this->ion_auth->_extra_where)->limit(1)->get($this->tables ['users']);
        ...
        if ($db_password === $old) {
            // store the new password and reset the remember code so all
            // remembered instances have to re-login
            $data = array(
                'password' => $new,
                'remember_code' => ''
            );
    ...
    public function username_check($username = '')
    {
        ...
        return $this->db->where('username', $username)->where($this->ion_auth->_extra_where)->count_all_results($this->tables ['users']) > 0;
    }
    ...
    public function email_check($email = '')
    {
        ...
        return $this->db->where('email', $email)->where($this->ion_auth->_extra_where)->count_all_results($this->tables ['users']) > 0;
    }
    ...
    public function register($username, $password, $email, $group_id = null, $additional_data = array(), $group_name = false)
    {
        ...
        if (!$group_id) {
            ...
            $group_id = ($group = $this->db->select('id')->where('name', $group_name)->get($this->tables ['groups'])->row()) ? $group->id : 0;
        }
        ...
        if(!empty($additional_data ['location_available_id'])){
            $data['location_available_id'] = $additional_data ['location_available_id'];
        }
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $this->db->select('username, email, id, password, group_id')->where(sprintf('(username = "%1$s" OR email = "%1$s")', $this->db->escape_str($identity)));

        if (isset($this->ion_auth->_extra_where)) {
            $this->db->where($this->ion_auth->_extra_where);
        }

        $query = $this->db->where('active', 1)->limit(1)->get($this->tables ['users']);
    ...
    public function force_login($user_id, $remember = FALSE)
    {
        ...
        $user = $this->db->where('active', 1)->limit(1)->get($this->tables ['users'])->row();
    ...
    public function _set_login($user, $remember)
    {
        ...
        $this->session->set_userdata(array(
            ...
            'email' => $user->email,
            'id' => $user->id, // kept for backwards
            // compatibility
            'user_id' => $user->id, // everyone likes to
            // overwrite id so we'll
            // use user_id
    ...
    public function get_users($group = false, $limit = null, $offset = null)
    {
        $this->db->select(array(
            $this->tables ['users'] . '.*',
            $this->tables ['groups'] . '.name AS ' . $this->db->protect_identifiers('group_name'),
            $this->tables ['groups'] . '.description AS ' . $this->db->protect_identifiers('group_description')
        ));
    ...
    public function get_user($id = NULL)
    {
        ...
        if (is_null($id) or is_bool($id)) {
            ...
            $_user_is_current = is_scalar($id) && $id ? array(
                $id
            ) :             // as bool is true, as array pass the value to log
    ...
    public function get_users_group($id = false)
    {
        ...
        $user = $this->db->select('group_id')->where('id', $id)->get($this->tables ['users'])->row();

        return $this->db->select('name, description')->where('id', $user->group_id)->get($this->tables ['groups'])->row();
    }
    ...
    public function delete_user($id)
    {
        ...
        $this->db->delete($this->tables ['meta'], array(
            $this->meta_join => $id
        ));
    ...
    public function login_remembered_user()
    {
        ...
        $query = $this->db->select($this->identity_column . ', id, group_id')->where($this->identity_column, get_cookie('identity'))->where('remember_code', get_cookie('remember_code'))->limit(1)->get($this->tables ['users']);
