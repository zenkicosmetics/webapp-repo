...
class Module_m extends MY_Model {
    ...
    /**
     * Get Return an array containing module data
     * 
     * @access public
     * @param string $module
    ...
    public function get($module_name = '') {
        ...
        $row = $this->db->where('modulename', $module_name)->get($this->_table)->row();
    ...
    /**
     * Get Modules Return an array of objects containing module related data
     * 
     * @param array $params
     *            The array containing the modules to load
     * @param bool $return_disabled
     *            Whether to return disabled modules
     * @access public
     * @return array
     */
    ...
    /**
     * Add Adds a module to the database
     * 
     * @access public
     * @param array $module
     *            the module
    ...
    /**
     * Update Updates a module in the database
     * 
     * @access public
     * @param array $slug
     *            Module slug to update
     * @param array $module
    ...
    /**
     * Delete Delete a module from the database
     * 
     * @param array $slug
     *            slug
     * @access public
    ...
    public function delete($slug) {
        return $this->db->delete($this->_table, array (
                'modulename' => $slug 
        ));
    }
    ...
    public function exists($module) {
        ...
        return $this->_module_exists [$module] = $this->db->where('modulename', $module)->count_all_results($this->_table) > 0;
    }
    ...
    public function enable($module) {
        if ($this->exists($module)) {
            $this->db->where('modulename', $module)->update($this->_table, array (
                    'enableflag' => 1 
            ));
            return TRUE;
        }
    ...
    public function disable($slug) {
        if ($this->exists($slug)) {
            $this->db->where('modulename', $slug)->update($this->_table, array (
                    'enableflag' => 0 
            ));
            return TRUE;
        }
    ...
    public function install($slug, $is_core = FALSE, $insert = FALSE) {
        ...
        // TURN ME ON BABY!
        $this->db->where('modulename', $slug)->update('Modules', array (
                'enableflag' => 1 
        ));
        ...
        foreach ( array (
            ...
            foreach ( $temp_modules as $path ) {
                ...
                if (in_array($slug, $known_array)) {
                    ...
                    if (file_exists($details_file) && filemtime($details_file) > $known_mtime [$slug] ['updated_on'] && $module = $this->_spawn_class($slug, $is_core)) {
                        list ( $class ) = $module;
    ...
    /**
     * Spawn Class Checks to see if a details.php exists and returns a class
     * 
     * @param string $slug
     *            name of the module
     * @access private
    ...
    private function _spawn_class($slug, $is_core = FALSE) {
        ...
        return class_exists($class) ? array (
                new $class(),
                dirname($details_file) 
        ) : FALSE;
    }
    
    /**
     * Help Retrieves help string from details.php
     * 
     * @param string $slug
     *            slug
     * @return bool
    ...
    /**
     * Roles Retrieves roles for a specific module
     * 
    ...
    /**
     * Help Retrieves version number from details.php
     * 
     * @param string $slug
     *            The module slug
     * @return bool
