...
class Admin extends Admin_Controller
{
	 ...
	 *
	 * @param	string	$module_name	The slug of the module to uninstall
	 * @access	public
	 * @return	void
	 */
	public function uninstall($module_name = '')
	{
		...
		if ($this->module_m->uninstall($module_name))
		{
			$this->session->set_flashdata('success', sprintf(lang('modules.uninstall_success'), $module_name));
			
			// Fire an event. A module has been disabled when uninstalled. 
			Events::trigger('module_disabled', $module_name);

			redirect('admin/modules');
		}

		$this->session->set_flashdata('error', sprintf(lang('modules.uninstall_error'), $module_name));
		redirect('admin/modules');
	}
	 ...
	 *
	 * @param	string	$module_name	The slug of the module to delete
	 * @access	public
	 * @return	void
	 */
	public function delete($module_name = '')
	{
		...
		if ($module_name == '/' OR $module_name == '*' OR empty($module_name))
		{
			show_error(lang('modules.module_not_specified'));
		}

		// lets kill this thing
		if ($this->module_m->uninstall($module_name) AND $this->module_m->delete($module_name))
		{
			$this->session->set_flashdata('success', sprintf(lang('modules.delete_success'), $module_name));

			$path = ADDONPATH . 'modules/' . $module_name;
		...
		$this->session->set_flashdata('error', sprintf(lang('modules.delete_error'), $module_name));
		redirect('admin/modules');
	}
	 ...
	 *
	 * @param	string	$module_name	The slug of the module to enable
	 * @access	public
	 * @return	void
	 */
	public function install($module_name)
	{
		if ($this->module_m->install($module_name))
		{
			// Fire an event. A module has been enabled when installed. 
			Events::trigger('module_enabled', $module_name);
							
			// Clear the module cache
			$this->pyrocache->delete_all('module_m');
			$this->session->set_flashdata('success', sprintf(lang('modules.install_success'), $module_name));
		}
		else
		{
			$this->session->set_flashdata('error', sprintf(lang('modules.install_error'), $module_name));
		}
	...
	{
		if ($this->module_m->enable($module_name))
		{
			...
			// Clear the module cache
			$this->pyrocache->delete_all('module_m');
			$this->session->set_flashdata('success', sprintf(lang('modules.enable_success'), $module_name));
		}
		else
		{
			$this->session->set_flashdata('error', sprintf(lang('modules.enable_error'), $module_name));
		}
	 ...
	 *
	 * @param	string	$module_name	The slug of the module to disable
	 * @access	public
	 * @return	void
	 */
	public function disable($module_name)
	{
		if ($this->module_m->disable($module_name))
		{
			...
			// Clear the module cache
			$this->pyrocache->delete_all('module_m');
			$this->session->set_flashdata('success', sprintf(lang('modules.disable_success'), $module_name));
		}
		else
		{
			$this->session->set_flashdata('error', sprintf(lang('modules.disable_error'), $module_name));
		}
	...
	public function upgrade($module_name)
	{
		...
		if ($this->module_m->upgrade($module_name))
		{
			// Fire an event. A module has been upgraded. 
			Events::trigger('module_upgraded', $module_name);
			
			$this->session->set_flashdata('success', sprintf(lang('modules.upgrade_success'), $module_name));
		}
		// If upgrade failed
		else
		{
			$this->session->set_flashdata('error', sprintf(lang('modules.upgrade_error'), $module_name));
		}
