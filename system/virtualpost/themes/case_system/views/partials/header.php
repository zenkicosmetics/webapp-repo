    ...
    <div class="ym-grid">
        ...
        <div class="ym-g50 ym-gr">
            <div class="top-nav-right">
                <!--<div ><a class="selected" href="#">For later ...</a></div> -->
                <?php if (APUtils::IsEnableCaseFunction()): ?>
                    <?php
                    if ($module == 'cases' && ($controller == 'cases' && in_array($method, array('index', 'under_construction', 'verification')) || ($controller == 'services' && $method == 'index') || ($controller == 'verification'))) {
                        $cssClass = 'selected';
                    } else {
                        $cssClass = '';
                    }
                    ?>
                    <div><a class="<?php echo $cssClass; ?>" href="<?php echo base_url() ?>cases"><?php language_e('them_case_syst_view_part_head_Servs')?></a></div>
                <?php endif; ?>
                <?php
                ...
                ?>
                <div><a class="<?php echo $cssClass; ?>" href="<?php echo base_url() ?>office"><?php language_e('them_case_syst_view_part_head_OffiSpac')?></a></div>
                <?php
                ...
                <?php
                if ($module == 'phones' && (($controller === 'phones' && in_array($method, array('index'))))) {
                    $cssClass = 'selected';
                } else {
                    $cssClass = '';
                }
                ?>
                <div><a class="<?php echo $cssClass; ?>" href="<?php echo base_url()?>phones" ><?php language_e('them_case_syst_view_part_head_Phon')?></a></div>
                <?php
                if ($module == 'mailbox' && (($controller === 'mailbox' && $method == 'index') || in_array($method, array('news', 'scans', 'instore', 'trash')))) {
                    $cssClass = 'selected';
                } else {
                    $cssClass = '';
                }
                ?>
                <div><a class="<?php echo $cssClass; ?>" href="<?php echo base_url() ?>mailbox" ><?php language_e('them_case_syst_view_part_head_Post')?></a></div>
            </div>
            ...
            <div  class="bottom-nav-right">
                ...
                <?php
                if (($module != 'info') && (($controller === 'account' || in_array($controller, array('index', 'users', 'postbox_setting', 'phone_setting', 'invoices', 'addresses', 'payment', 'cloud'))))) {
                    $cssClass = 'selected';
                } else {
                    $cssClass = '';
                }
            ...
            <div id="top-search">
                <input type="text" id="mainSearchTextbox" placeholder="<?php language_e('them_case_syst_view_part_head_Seac')?>" /><i class="fa fa-search icon-search" aria-hidden="true"></i>
                <input type="submit" value="Search" />
            </div>
        </div>
    </div>
</header>
<div class="ym-grid COLOR_002" id="header_banner"  style="height: 10px"></div>
