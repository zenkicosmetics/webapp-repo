...
(function($) {
    ...
    $.confirmPayment = function(config) {
        ...
        $("<div title='" + config.title + "' style='font-weight: bold; color: #0089C8; padding: 20px 10px;'><p>" + config.message + "</p></div>").dialog({
            ...
            buttons: [
                {
                	text: config.okText,
                	"class": 'sub-button-dialog understand',
	                click: function() {
	                    config.yes();
	                    $(this).dialog("destroy");
	                }
    ...
    $.error = function(config) {
        ...
        var dialog = $("<div title='" + config.title + "' style='font-weight: bold; color: #d14b4b; padding: 10px 10px;'><p>" + config.message + "</p></div>").dialog({
            close: function(event, ui) { $(this).parents(".ui-dialog").remove(); },
            bgiframe: true,
            resizable: false,
            width: config.width,
