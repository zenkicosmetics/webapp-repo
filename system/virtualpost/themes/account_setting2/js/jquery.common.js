...
(function($) {
    ...
    $.bindSelect = function(url, param, select_control_id, label_default, value_default, callback) {
    	...
    	$.post(url, param, function(data) {
            ...
            sel.empty();
            if (label_default != null) {
                sel.append('<option value="" disabled>' + label_default + '</option>');
            }
            var first_item = value_default;
            for (var i=0; i<data.length; i++) {
                if(i == 0){
                    first_item = data[i].key;
                }
	...
	$.autoFitScreenL = function (width, obj){
        // Gets screen width
        var screen_width = $(window).width() - 280;
        if(screen_width > 1366){
            screen_width = 1366;
        }
        if (screen_width > width) {
    ...
    $.openDialog = function(divId, config){
        ...
        if((!$.isEmpty(config.show_only_close_button) && config.show_only_close_button == true)){
