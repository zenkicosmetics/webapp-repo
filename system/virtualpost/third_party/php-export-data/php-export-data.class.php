...
class ExportDataXML extends ExportData {
	...
	private $_firt_row = array();
	
	private $_check_first_row = false;
	...
	function generateRow($row) {
	    ...
	    if(!$this->_check_first_row){
 	    ...
 	    if($this->_check_first_row){
    		$output .= "        <Row>\n";
    		foreach ($row as $k => $v) {
    			$output .= $this->generateNode($k,$v);
    		}
    		$output .= "        </Row>\n";
 	    }
 	    $this->_check_first_row = true;
		return $output;
	}

	private function generateNode($key, $item) {
	    $output = '';
	    $item = str_replace('&#039;', '&apos;', htmlspecialchars($item, ENT_QUOTES));
	    $output .= "            ";
	    $output .= "<". $this->_firt_row[$key] . ">";
	    $output .= $item;
	    $output .= "</" . $this->_firt_row[$key] . ">\n";
	    return $output;
	}
	...
	function sendHttpHeaders() {
		header("Content-Type: application/csv; charset=" . $this->encoding);
		header("Content-Disposition: inline; filename=\"" . basename($this->filename) . "\"");
	}
...
class ExportDataFactory{
    ...
    public function getInstance(){
        if(in_array($this->exportTo, array('browser','file','string') )) 
        {
            if ($this->type == 'csv')
            {
                return new ExportDataCSV($this->exportTo, $this->filename, $this->type);
            }
            else if ($this->type == 'tsv')
            {
                return new ExportDataTSV($this->exportTo, $this->filename, $this->type);
            }
            else if ($this->type == 'xls')
            {
                return new ExportDataExcel($this->exportTo, $this->filename, $this->type);
            }
            else if ($this->type == 'xml')
            {
            	return new ExportDataXML($this->exportTo, $this->filename, $this->type);
            }
        }else{
            return NULL;
        }
